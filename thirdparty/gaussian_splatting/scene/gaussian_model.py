...
class GaussianModel:
    def __init__(self, sh_degree: int, config=None):
        ...
        # Keep log-scales in a numerically safe range before applying exp.
        # This prevents NaN/Inf propagation during long online optimization runs.
        self.min_log_scaling = -10.0
        self.max_log_scaling = 6.0
        self.default_log_scaling = -3.0
    ...
    def _sanitize_log_scaling(self, scaling_tensor):
        scaling_tensor = torch.nan_to_num(
            scaling_tensor,
            nan=self.default_log_scaling,
            posinf=self.max_log_scaling,
            neginf=self.min_log_scaling,
        )
        return torch.clamp(
            scaling_tensor,
            min=self.min_log_scaling,
            max=self.max_log_scaling,
        )

    @torch.no_grad()
    def sanitize_parameters_(self):
        self._scaling.data.copy_(self._sanitize_log_scaling(self._scaling.data))
        ...
        for group in self.optimizer.param_groups:
            ...
            for key in ("exp_avg", "exp_avg_sq"):
                if key in state:
                    state[key] = torch.nan_to_num(state[key], nan=0.0, posinf=0.0, neginf=0.0)

    @property
    def get_scaling(self):
        return self.scaling_activation(self._sanitize_log_scaling(self._scaling))
    ...
    def create_pcd_from_image(self, cam_info, init=False, scale=2.0, depthmap=None, privacy_mask=None):
        cam = cam_info
        ...
        return self.create_pcd_from_image_and_depth(cam, rgb, depth, init, privacy_mask)

    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False, privacy_mask=None):
        """
        Create point cloud from RGB-D image.

        Args:
            cam: Camera object with intrinsics and pose
            rgb: RGB image (o3d.geometry.Image)
            depth: Depth image (o3d.geometry.Image)
            init: Whether this is initialization
            privacy_mask: Optional privacy mask (H, W) where True = private pixel to exclude
        ...
        # ====== Privacy-Preserving SLAM: Filter private pixels ======
        # Zero out depth in private regions to prevent Gaussian creation there
        if privacy_mask is not None:
            # Convert o3d.Image to numpy, apply mask, convert back
            depth_np = np.asarray(depth).copy()
            # Handle both torch tensor and numpy privacy_mask
            if hasattr(privacy_mask, 'cpu'):
                mask_np = privacy_mask.cpu().numpy()
            else:
                mask_np = privacy_mask
            ...
            # Zero out private pixels (project_valid_depth_only=True will skip these)
            depth_np[mask_np > 0.5] = 0.0
    ...
    def extend_from_pcd_seq(
        self, cam_info, kf_id=-1, init=False, scale=2.0, depthmap=None, privacy_mask=None
    ):
        fused_point_cloud, features, scales, rots, opacities = (
            self.create_pcd_from_image(cam_info, init, scale=scale, depthmap=depthmap, privacy_mask=privacy_mask)
        )
    ...
    def save_ply_sanitized(self, path, private_mask=None):
        """
        Save PLY with all private Gaussians removed.

        For privacy-preserving SLAM: removes any remaining tagged private
        Gaussians before final export.

        Args:
            path: Output PLY file path
            private_mask: Optional boolean tensor (N,) where True = private Gaussian to remove.
                         If None, uses self.private_flag if available.
        """
        # Get private mask
        if private_mask is None:
            private_mask = getattr(self, 'private_flag', None)

        # Prune private Gaussians if mask exists
        if private_mask is not None and private_mask.any():
            num_private = private_mask.sum().item()
            print(f"[save_ply_sanitized] Removing {num_private} private Gaussians before export")
            self.prune_points(private_mask)

        # Now save the sanitized PLY
        self.save_ply(path)
