...
class Mapper(object):
    ...
    def __init__(
        ...
        # ====== Privacy-Preserving SLAM: Initialize simultaneous mode detector ======
        self.privacy_manager = None
        privacy_cfg = self.config.get("privacy", {})
        if privacy_cfg.get("enable", False):
            privacy_mode = privacy_cfg.get("mode", "postprocess")
            if privacy_mode in ["simultaneous", "hybrid"]:
                try:
                    from src.privacy import PrivacyManager
                    self.privacy_manager = PrivacyManager(privacy_cfg, self.device)
                    self.printer.print(
                        f"[Mapper] Privacy simultaneous mode enabled ({privacy_mode})",
                        FontColor.INFO
                    )
                except Exception as e:
                    self.printer.print(
                        f"[Mapper] Failed to initialize PrivacyManager: {e}",
                        FontColor.ERROR
                    )
                    self.privacy_manager = None
    ...
    def run(self):
        ...
        while True:
            ...
            # ====== Privacy-Preserving SLAM: Detect privacy regions for this keyframe ======
            if self.privacy_manager is not None:
                try:
                    # Get the image from viewpoint
                    image = viewpoint.original_image  # (3, H, W)
                    privacy_mask = self.privacy_manager.detect_realtime(image, frame_idx)
                    if privacy_mask is not None:
                        viewpoint.privacy_mask = privacy_mask
                        if self.verbose:
                            num_private_pixels = (privacy_mask > 0.5).sum().item()
                            total_pixels = privacy_mask.numel()
                            self.printer.print(
                                f"[Privacy] Frame {frame_idx}: {num_private_pixels}/{total_pixels} "
                                f"({100*num_private_pixels/total_pixels:.1f}%) private pixels",
                                FontColor.INFO
                            )
                except Exception as e:
                    if self.verbose:
                        self.printer.print(f"[Privacy] Detection failed: {e}", FontColor.ERROR)
            ...
            self.gaussians.extend_from_pcd_seq(
                ...
                privacy_mask=viewpoint.privacy_mask  # Filter private pixels from Gaussian creation
            )
    ...
    def get_cameras_dict(self) -> dict:
        """
        Get dictionary of all Camera objects.

        Used by privacy post-processing to access all keyframe cameras.

        Returns:
            Dict mapping video_idx to Camera object
        """
        return self.cameras
    ...
    def initialize_mapper(self, cur_video_idx):
        ...
        for video_idx in range(cur_video_idx + 1):
            ...
            # ====== Privacy-Preserving SLAM: Detect privacy regions for init frames ======
            if self.privacy_manager is not None:
                try:
                    image = viewpoint.original_image  # (3, H, W)
                    privacy_mask = self.privacy_manager.detect_realtime(image, frame_idx)
                    ...
                    pass  # Silently continue during initialization
            ...
            self.gaussians.extend_from_pcd_seq(
                ...
                privacy_mask=viewpoint.privacy_mask
            )
    ...
    def initialize_map_opt(self):
        ...
        for mapping_iteration in range(self.init_itr_num):
            ...
            if not self.uncertainty_aware:
                ...
                # ====== Privacy-Preserving SLAM: Inject high uncertainty for privacy regions ======
                if self.privacy_manager is not None and hasattr(viewpoint, 'privacy_mask') and viewpoint.privacy_mask is not None:
                    current_uncertainty = self.privacy_manager.inject_privacy_uncertainty(
                        current_uncertainty, viewpoint.privacy_mask
                    )
                ...
                # Add uncertainty regularization to prevent trivial "everything uncertain" solution
                if self.config["mapping"]["uncertainty_params"].get("uncertainty_regularization", {}).get("enabled", True):
                    uncertainty_reg_loss = map_utils.compute_uncertainty_regularization_loss(
                        uncertainty=current_uncertainty,
                        opacity=opacity.squeeze(0) if opacity.dim() == 3 else opacity,
                        regularization_config=self.config["mapping"]["uncertainty_params"].get("uncertainty_regularization", {})
                    )
                    loss_init += uncertainty_reg_loss

                # Add depth-uncertainty correlation penalty if enabled
                if self.config["mapping"]["uncertainty_params"].get("correlation_penalty", {}).get("enabled", False):
                    correlation_penalty = map_utils.compute_depth_uncertainty_correlation_penalty(
                        rendered_depth=depth,
                        uncertainty=current_uncertainty,
                        correlation_config=self.config["mapping"]["uncertainty_params"]["correlation_penalty"],
                        opacity=opacity
                    )
                    loss_init += correlation_penalty
        ...
        for cur_iter in range(iters):
            ...
            if not self.uncertainty_aware:
                ...
                if self.iterations_after_densify_or_reset >= 20:
                    ...
                    # Add uncertainty regularization to prevent trivial solution
                    if self.config["mapping"]["uncertainty_params"].get("uncertainty_regularization", {}).get("enabled", True):
                        uncertainty_reg_loss = map_utils.compute_uncertainty_regularization_loss(
                            uncertainty=current_uncertainty,
                            opacity=opacity.squeeze(0) if opacity.dim() == 3 else opacity,
                        ...
                        loss_mapping += uncertainty_reg_loss
                    ...
                    if self.config["mapping"]["uncertainty_params"].get("correlation_penalty", {}).get("enabled", False):
                        ...
                        # print("Depth-uncertainty correlation penalty:", correlation_penalty.item())
                        loss_mapping += correlation_penalty
    ...
    def save_fig_everything(self, keyframe_idx: int, plot_dir: str, suffix: str = "", depth_max: float = 10.0):
        ...
        with open(f"{plot_dir}/psnr_log.txt", "a") as f:
            f.write(f"Keyframe idx: {keyframe_idx}, PSNR: {psnr_score.item():.6f}\n")
        with open(f"{plot_dir}/uncertainty_ssim_log.txt", "a") as f:
            f.write(
                f"Keyframe idx: {keyframe_idx}, Uncertainty mean: {uncertainty_map.mean().item():.6f}, SSIM loss mean: {ssim_loss.mean().item():.6f}\n"
            )
        # save rendered_image, gt, gt_image, pred to a npz file
        try:
            np.savez_compressed(f"{plot_dir}/{keyframe_idx}{suffix}.npz",
                                gt_image=gt_image.cpu().numpy(),
                                gt_depth=gt_depth,
                                rendered_image=rendered_img.cpu().numpy(),
                                opacity=opacity.cpu().numpy() if opacity is not None else None)
        except Exception as e:
            print(f"[Mapper] Error saving npz file: {e}")
            np.savez(f"{plot_dir}/{keyframe_idx}{suffix}.npz",
                                gt_image=gt_image.cpu().numpy(),
                                gt_depth=gt_depth,
                                rendered_image=rendered_img.cpu().numpy())
