...
class FactorGraph:
    ...
    def update(self, t0=None, t1=None, itrs=2, use_inactive=False,
        ...
        with torch.amp.autocast('cuda',enabled=False):
            ...
            # bundle adjustment
            # Note: update() runs under @torch.no_grad() for efficiency. However,
            # the IMU prior refinement inside Video.ba requires autograd to be
            # enabled so we temporarily enable gradients only for this call.
    ...
    def add_backend_proximity_factors(self, t_start, t_end, nms, radius, thresh, max_factors, beta, t_start_loop=None, loop=False):
        """
        Add proximity factors for backend BA, with optional FAISS-based loop closure.
        
        Stage 6: If MambaVision descriptors are enabled and loop=True, uses FAISS to
        quickly identify loop closure candidates, then verifies them geometrically.
        """
        if t_start_loop is None or not loop:
            t_start_loop = t_start
        assert t_start_loop >= t_start, f'short: {t_start_loop}, long: {t_start}.'
        
        # Stage 6: FAISS-accelerated loop closure
        if loop and self.video.use_mamba_descriptors and self.video.faiss_index is not None:
            return self._add_loop_factors_with_faiss(
                t_start, t_end, nms, thresh, max_factors, beta, t_start_loop
            )

        # Original exhaustive search fallback
        import time
        ...
        d = self.video.distance(ii, jj, beta=beta)
        distance_time = time.time() - t0_distance
        ...
        if len(es) < 3 or (loop and loop_edges==0):
            exhaustive_total_time = (time.time() - exhaustive_start) * 1000  # ms
            print(f"[FactorGraph] Exhaustive loop closure completed:")
            print(f"  - Candidates evaluated: {len(ii)}")
            print(f"  - Loop edges added: 0")
            print(f"  - Total edges: 0")
            print(f"  - Timing: Distance {distance_time*1000:.1f}ms, Total {exhaustive_total_time:.1f}ms")
            return 0
        ...
        edge_time = time.time() - t0_edges
        
        exhaustive_total_time = (time.time() - exhaustive_start) * 1000  # ms
        
        # Log exhaustive search results for comparison with FAISS
        print(f"[FactorGraph] Exhaustive loop closure completed:")
        print(f"  - Candidates evaluated: {len(ii)}")
        print(f"  - Loop edges added: {loop_edges}")
        print(f"  - Total edges: {edge_num}")
        print(f"  - Timing: Distance {distance_time*1000:.1f}ms, Edges {edge_time*1000:.1f}ms, Total {exhaustive_total_time:.1f}ms")

        return edge_num
    
    def _add_loop_factors_with_faiss(self, t_start, t_end, nms, thresh, max_factors, beta, t_start_loop):
        """
        Stage 6: Use FAISS for fast loop closure candidate retrieval + geometric verification.
        
        This provides significant speedup compared to exhaustive search:
        - FAISS query: O(log N) vs. O(N²) exhaustive
        - Geometric verification only on top-K candidates
        
        Args:
            t_start: Global BA window start
            t_end: Global BA window end (exclusive)
            nms: Non-maximum suppression radius after adding edge
            thresh: Photometric distance threshold for geometric verification
            max_factors: Maximum number of edges to add
            beta: Weighting parameter for distance computation
            t_start_loop: Local window start for loop detection
        
        Returns:
            int: Number of edges added to the factor graph
        """
        import time
        import numpy as np
        timing_stats = {
            'faiss_query': 0.0,
            'reranking': 0.0,
            'geometric_verify': 0.0,
            'edge_construction': 0.0
        }

        # Configuration from video settings
        topk = getattr(self.video, 'loop_topk', 20)
        min_temporal_distance = getattr(self.video, 'loop_min_temporal_dist', 30)
        descriptor_sim_threshold = getattr(self.video, 'loop_descriptor_threshold', 0.7)

        # Check if local reranking is enabled
        use_local_reranking = getattr(self.video, 'use_local_reranking', False)

        es = []
        loop_edges = 0
        n_neighboring = 1

        # Local window edges (always included)
        radius = 1  # Use small radius for local connectivity
        for i in range(t_start_loop, t_end):
        ...
        rejected_by_descriptor = 0
        rejected_by_geometry = 0
        reranked_count = 0
        
        # Query FAISS for each frame in the loop window
        for i in range(t_start_loop, t_end):
            if len(es) >= max_factors:
                break
            ...
            # FAISS query
            t0 = time.time()
            candidates = self.video.query_loop_candidates(
                frame_idx=i,
                topk=topk,
                min_temporal_distance=min_temporal_distance,
                max_candidates=topk
            )
            timing_stats['faiss_query'] += time.time() - t0
            faiss_candidate_count += len(candidates)
            
            if len(candidates) == 0:
                continue
            
            # Filter by descriptor similarity threshold
            high_sim_candidates = [(j, sim) for j, sim in candidates if sim >= descriptor_sim_threshold]
            rejected_by_descriptor += (len(candidates) - len(high_sim_candidates))
            
            if len(high_sim_candidates) == 0:
                continue

            # Stage 2: MNN Reranking (if enabled and local features available)
            if use_local_reranking and self.video.local_valid[i]:
                t0_rerank = time.time()

                # Get query local features
                query_local = self.video.get_local_features(i).numpy()

                if len(query_local) > 0:
                    from src.modules.mamba_descriptor.reranker import compute_mnn_scores, blend_with_global

                    # Get candidate frame IDs and global similarity scores
                    candidate_frame_ids = [j for j, _ in high_sim_candidates]
                    global_scores = np.array([sim for _, sim in high_sim_candidates])

                    # Get candidate local features (batch lookup)
                    candidate_locals = self.video.get_local_features_batch(candidate_frame_ids)

                    # Compute MNN scores and blend with global
                    mnn_scores = compute_mnn_scores(
                        query_local, candidate_locals, self.video.rerank_t2
                    )
                    blended_scores = blend_with_global(
                        mnn_scores, global_scores, self.video.rerank_global_weight
                    )

                    # Re-sort candidates by blended score (descending)
                    rerank_order = np.argsort(-blended_scores)
                    high_sim_candidates = [
                        (candidate_frame_ids[r], blended_scores[r])
                        for r in rerank_order
                    ]

                    reranked_count += 1

                timing_stats['reranking'] += time.time() - t0_rerank

            # Geometric verification: compute photometric distance
            t0 = time.time()
            for j, sim in high_sim_candidates:
                if len(es) >= max_factors:
                    break
                
                # Ensure j is within BA window
                if j < t_start or j >= t_end:
                    continue
                
                # Compute photometric distance for verification
                ii_tensor = torch.tensor([i], device=self.device)
                jj_tensor = torch.tensor([j], device=self.device)
                dist = self.video.distance(ii_tensor, jj_tensor, beta=beta).item()
                
                # Accept if photometric distance is below threshold
                if dist <= thresh:
                    # Add bidirectional edges with neighbors for robustness
                    sub_es = []
                    for si in range(max(i-n_neighboring, t_start_loop), min(i+n_neighboring+1, t_end)):
                        for sj in range(max(j-n_neighboring, t_start), min(j+n_neighboring+1, t_end)):
                            if si != sj and abs(si - sj) > min_temporal_distance:
                                sub_es.append((si, sj))
            ...
            timing_stats['geometric_verify'] += time.time() - t0
        
        # Construct factor graph edges
        t0 = time.time()
        if len(es) < 3:
            print(f"[FactorGraph] FAISS loop closure: insufficient edges ({len(es)})")
            return 0
        
        ii, jj = torch.tensor(es, device=self.device).unbind(dim=-1)
        self.add_factors(ii, jj, remove=True)
        edge_num = len(self.ii)
        timing_stats['edge_construction'] = time.time() - t0
        
        # Report statistics
        total_time = sum(timing_stats.values()) * 1000  # ms
        
        print(f"[FactorGraph] FAISS loop closure completed:")
        print(f"  - FAISS candidates: {faiss_candidate_count}")
        if use_local_reranking:
            print(f"  - Reranked queries: {reranked_count}")
        print(f"  - Rejected by descriptor (<{descriptor_sim_threshold:.2f}): {rejected_by_descriptor}")
        print(f"  - Rejected by geometry (>{thresh:.1f}): {rejected_by_geometry}")
        print(f"  - Verified candidates: {verified_candidate_count}")
        print(f"  - Loop edges added: {loop_edges}")
        print(f"  - Total edges: {edge_num}")
        if use_local_reranking:
            print(f"  - Timing: FAISS {timing_stats['faiss_query']*1000:.1f}ms, "
                  f"Rerank {timing_stats['reranking']*1000:.1f}ms, "
                  f"Verify {timing_stats['geometric_verify']*1000:.1f}ms, "
