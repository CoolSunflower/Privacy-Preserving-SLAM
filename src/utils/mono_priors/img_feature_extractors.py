...
class Fit3DModels(torch.nn.Module):
    ...
    def get_intermediate_layers(
        ...
        # timm 1.0+ removed _intermediate_layers; delegate to the public API
        need_prefix = return_prefix_tokens or return_class_token
        outputs = self.model.get_intermediate_layers(
            x, n, reshape=reshape, return_prefix_tokens=need_prefix, norm=norm
        )

        if need_prefix:
            if return_class_token:
                # timm returns (spatial, all_prefix_tokens) tuples;
                # callers expect (spatial, class_token) where class_token is prefix[:, 0]
                return tuple((spatial, prefix[:, 0]) for spatial, prefix in outputs)
            return tuple(outputs)
        return tuple(outputs)
