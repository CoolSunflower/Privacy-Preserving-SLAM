...
def align_kf_traj(npz_path,stream,return_full_est_traj=False,printer=None):
    ...
    # Check if stream.poses is a dict (indexed by timestamp) or list (indexed by frame index)
    poses_is_dict = isinstance(stream.poses, dict)
    
    for i in range(video_timestamps.shape[0]):
        timestamp = int(video_timestamps[i])
        
        # Handle both dict (timestamp-indexed) and list (frame-indexed) poses
        if poses_is_dict:
            # Dictionary: video timestamp should match GT timestamp
            if timestamp not in stream.poses:
                # Video has frame index, need to map to actual timestamp
                if hasattr(stream, 'image_timestamps') and timestamp < len(stream.image_timestamps):
                    actual_timestamp = int(stream.image_timestamps[timestamp])
                    if actual_timestamp in stream.poses:
                        gt_pose = stream.poses[actual_timestamp]
                    else:
                        if printer:
                            printer.print(f'GT pose not found for frame {timestamp} (timestamp {actual_timestamp}), skipping!', FontColor.INFO)
                        continue
                else:
                    if printer:
                        printer.print(f'Timestamp {timestamp} not found in GT poses, skipping {i}th pose!', FontColor.INFO)
                    continue
            else:
                gt_pose = stream.poses[timestamp]
        else:
            # List: direct frame index lookup
            if timestamp >= len(stream.poses):
                if printer:
                    printer.print(f'Frame index {timestamp} out of range, skipping {i}th pose!', FontColor.INFO)
                continue
            gt_pose = stream.poses[timestamp]
        
        # Check for NaN/Inf
        val = gt_pose.sum() if hasattr(gt_pose, 'sum') else np.sum(gt_pose)
...
def align_full_traj(traj_est_full,stream,printer):
    ...
    # Get the length - for dict use image_timestamps, for list use len(poses)
    if poses_is_dict:
        if not hasattr(stream, 'image_timestamps'):
            printer.print('ERROR: stream.poses is dict but no image_timestamps available', FontColor.ERROR)
            return None, None, None, None, None
        n_frames = len(stream.image_timestamps)
    else:
        n_frames = len(stream.poses)
    
    for i in range(n_frames):
        # Get GT pose - handle both dict and list
        if poses_is_dict:
            # Map frame index to actual timestamp
            actual_timestamp = int(stream.image_timestamps[i])
            if actual_timestamp not in stream.poses:
                if i % 10 == 0 or i < 5:  # Log occasionally to avoid spam
                    printer.print(f'GT pose not found for frame {i} (timestamp {actual_timestamp}), skipping!', FontColor.INFO)
                continue
            gt_pose = stream.poses[actual_timestamp]
        else:
