...
try:
    ...
    PEARSON_AVAILABLE = True
except ImportError:
    PEARSON_AVAILABLE = False
    print("[WARNING] torchmetrics not available. Pearson depth loss disabled.")
...
def get_loss_tracking_rgb(config, image, opacity, viewpoint, uncertainty=None):
    ...
    if uncertainty is not None:
        ...
        # Contribution 3: Check for cycle-aware weighting (Idea A)
        cycle_aware_config = config.get("uncertainty_params", {}).get("cycle_aware", {})
        if cycle_aware_config.get("enabled", False):
            # Use minimum floor instead of hard zeroing
            min_weight = cycle_aware_config.get("min_weight", 0.05)
            weights = torch.clamp(weights, min=min_weight)
        else:
            # Original behavior: hard threshold zeroing
...
def get_loss_mapping_rgbd(config, image, depth, viewpoint):
    ...
    # Add Pearson depth loss if enabled
    # Compares rendered depth with GT depth using scale-invariant correlation
    if config["Training"].get("use_pearson_depth", False) and PEARSON_AVAILABLE:
        pearson_loss_value = pearson_depth_loss(
            depth, gt_depth, mask=depth_pixel_mask, invert_estimate=False
        )
        pearson_weight = config["Training"].get("pearson_weight", 0.001)
        loss += pearson_weight * pearson_loss_value
...
def get_loss_mapping_uncertainty(
    ...
    # Privacy-Preserving SLAM: Inject high uncertainty for private regions
    # Private regions get high uncertainty (beta) which down-weights their contribution
    # to the mapping loss, preventing private content from being encoded into Gaussians
    if viewpoint.privacy_mask is not None:
        privacy_beta = config.get("privacy", {}).get("uncertainty_beta", 100.0)
        # Resize privacy mask to match uncertainty dimensions
        privacy_mask_resized = F.interpolate(
            viewpoint.privacy_mask.unsqueeze(0).unsqueeze(0).float(),
            uncertainty.shape[-2:],
            mode='bilinear',
            align_corners=False
        ).squeeze()
        # Override uncertainty in private regions with high beta
        uncertainty = torch.where(
            privacy_mask_resized > 0.5,
            torch.full_like(uncertainty, privacy_beta),
            uncertainty
        )
    ...
    # ====== Contribution 3: Cycle-Aware Uncertainty Refinement (Idea A + B) ======
    # Idea A: Minimum weight floor to guarantee gradient flow and break self-reinforcing cycles
    # Idea B: Trend-aware modulation to accelerate recovery for converging patches

    cycle_aware_config = config["uncertainty_params"].get("cycle_aware", {})
    use_cycle_aware = cycle_aware_config.get("enabled", False)

    # 1. Compute raw weights
    weights = 0.5 / (uncer_resized.unsqueeze(0))**2

    if use_cycle_aware:
        # Idea A: Apply minimum floor instead of hard zeroing
        min_weight = cycle_aware_config.get("min_weight", 0.05)
        weights = torch.clamp(weights, min=min_weight)

        # Idea B: Trend-aware modulation - boost weights for recovering patches
        use_trend = cycle_aware_config.get("use_trend_modulation", True)
        if use_trend and viewpoint.beta_ema is not None:
            ema_alpha = cycle_aware_config.get("ema_alpha", 0.3)
            trend_boost_factor = cycle_aware_config.get("trend_boost_factor", 2.0)
            trend_threshold = cycle_aware_config.get("trend_threshold", -0.1)

            # Compute trend: negative means β is decreasing (recovering)
            beta_trend = uncertainty.detach() - viewpoint.beta_ema

            # Resize trend to match weight dimensions
            trend_resized = F.interpolate(
                beta_trend.unsqueeze(0).unsqueeze(0),
                weights.shape[-2:],
                mode='bilinear',
                align_corners=False
            ).squeeze(0)

            # Boost weights where β is decreasing below threshold (recovering regions)
            # trend < threshold → boost by trend_boost_factor
            # trend >= threshold → no boost (1.0x)
            boost = torch.where(
                trend_resized < trend_threshold,
                torch.tensor(trend_boost_factor, device=weights.device),
        ...
        ema_alpha = cycle_aware_config.get("ema_alpha", 0.3)
        if viewpoint.beta_ema is None:
            viewpoint.beta_ema = uncertainty.detach().clone()
        else:
            viewpoint.beta_ema = (1 - ema_alpha) * viewpoint.beta_ema + ema_alpha * uncertainty.detach()
    else:
        # Original behavior: hard threshold zeroing (for backward compatibility)
        weights = torch.where(weights < 0.1, 0.0, weights)
...
def pearson_depth_loss(rendered_depth, reference_depth, mask=None, invert_estimate=True):
    """Compute Pearson correlation-based depth loss.
    
    This loss measures the correlation between rendered depth and reference depth
    (monocular estimate or GT depth). It is scale-invariant, making it ideal for
    monocular depth which has unknown absolute scale.
    
    Loss = 1 - PearsonCorrelationCoefficient
    
    Perfect correlation: loss = 0
    No correlation: loss = 1
    Anti-correlation: loss = 2
    
    Args:
        rendered_depth: Rendered depth map (1, H, W) or (H, W)
        reference_depth: Reference depth (monocular or GT) (1, H, W) or (H, W)
        mask: Optional valid pixel mask (1, H, W) or (H, W)
        invert_estimate: If True, tries multiple depth inversions to handle scale ambiguity
                        (MM3DGS-SLAM compatibility for monocular depth)
    
    Returns:
        Scalar loss tensor (0.0 if conditions not met)
    """
    if not PEARSON_AVAILABLE:
        return torch.tensor(0.0, device=rendered_depth.device, requires_grad=True)
    
    # Flatten tensors and apply mask
    rendered_flat = rendered_depth.flatten()
    reference_flat = reference_depth.flatten()
    
    if mask is not None:
        mask_flat = mask.flatten().bool()
        rendered_flat = rendered_flat[mask_flat]
        reference_flat = reference_flat[mask_flat]
    
    # Safety check: empty tensors after masking
    if rendered_flat.numel() == 0 or reference_flat.numel() == 0:
        return torch.tensor(0.0, device=rendered_depth.device, requires_grad=True)
    
    # Safety check: need at least 2 elements for correlation
    if rendered_flat.numel() < 2:
        return torch.tensor(0.0, device=rendered_depth.device, requires_grad=True)
    
    # Safety check: avoid NaN from zero variance
    if rendered_flat.std() < 1e-6 or reference_flat.std() < 1e-6:
        return torch.tensor(0.0, device=rendered_depth.device, requires_grad=True)
    
    # Compute correlation-based loss
    if invert_estimate:
        # Try multiple inversions to handle monocular depth scale ambiguity
        # This matches MM3DGS-SLAM's approach for monocular depth
        loss1 = (1 - pearson_corrcoef(-reference_flat, rendered_flat)).mean()
        loss2 = (1 - pearson_corrcoef(1 / (reference_flat + 200.0), rendered_flat)).mean()
        loss = torch.min(loss1, loss2)
    else:
        loss = (1 - pearson_corrcoef(reference_flat, rendered_flat)).mean()
